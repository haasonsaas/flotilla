...
async fn node(name: &str, port: u16, peers: Vec<(&str, u16)>, dir: &std::path::Path) -> Node {
    let cfg = Config {
        ...
        static_identity: Some(StaticIdentityConfig {
            ...
            peers: peers
                .into_iter()
                .map(|(n, p)| StaticPeer { node_id: format!("id-{n}"), name: n.into(), ips: vec!["127.0.0.1".into()], port: Some(p) })
                .collect(),
        }),
        ..Config::default()
    };
    ...
    let store = Arc::new(Store::open(&cfg.data_dir.join("store.redb"), me.node_id.clone()).unwrap());
    ...
    Node { state, base: format!("http://127.0.0.1:{port}"), http: reqwest::Client::new() }
}
...
async fn two_nodes_sync_facts_and_records() {
    ...
    eventually("facts replicated", || {
        a.state.store.get(&keys::node_facts("id-b")).unwrap().is_some() && b.state.store.get(&keys::node_facts("id-a")).unwrap().is_some()
    })
    .await;
    ...
    let r: flotilla_core::Record = a
        ...
        .json(&PutRecordRequest { value: serde_json::json!({"x": 1}) })
        .send()
        .await
        .unwrap()
        .json()
    ...
    eventually("record replicated", || b.state.store.get("test/hello").unwrap().map(|r| r.value == serde_json::json!({"x": 1})).unwrap_or(false)).await;

    // Status on B lists both nodes as online with labels.
    let st: StatusResponse = b.http.get(format!("{}/v1/status", b.base)).send().await.unwrap().json().await.unwrap();
    ...
    b.http.delete(format!("{}/v1/records/test/hello", b.base)).send().await.unwrap().error_for_status().unwrap();
    eventually("tombstone replicated", || a.state.store.get("test/hello").unwrap().is_none()).await;
...
async fn exec_streams_frames() {
    ...
    let resp = a
        ...
        .json(&ExecRequest { cmd: vec!["sh".into(), "-c".into(), "echo one; echo two 1>&2; exit 4".into()], ..Default::default() })
        .send()
        .await
        .unwrap();
    ...
    assert!(frames.contains(&ExecFrame::Stdout { data: "one\n".into() }));
    assert!(frames.contains(&ExecFrame::Stderr { data: "two\n".into() }));
...
async fn job_is_claimed_run_on_selected_node_and_result_replicates() {
    ...
    let a = node("a", pa, vec![("b", pb)], &dir).await;
    let b = node("b", pb, vec![("a", pa)], &dir).await;
    eventually("facts replicated", || a.state.store.get(&keys::node_facts("id-b")).unwrap().is_some()).await;

    let id = uuid::Uuid::new_v4().to_string();
    let spec = JobSpec {
        id: id.clone(),
        cmd: vec!["sh".into(), "-c".into(), "echo hello from $(hostname); echo line2".into()],
        cwd: None,
        env: Default::default(),
    ...
    eventually("result replicated to submitter", || a.state.store.get(&keys::result(&id)).unwrap().is_some()).await;
    let result: JobResult = a.state.store.get(&keys::result(&id)).unwrap().unwrap().parse().unwrap();
    ...
    let claim: JobClaim = a.state.store.get(&keys::claim(&id)).unwrap().unwrap().parse().unwrap();
    ...
    let log = b.http.get(format!("{}/v1/jobs/{id}/log", b.base)).send().await.unwrap();
    assert_eq!(log.status(), 200);
    assert!(log.text().await.unwrap().contains("line2"));
    let missing = a.http.get(format!("{}/v1/jobs/{id}/log", a.base)).send().await.unwrap();
...
async fn job_cancel_kills_running_process() {
    ...
    a.state.store.put_json(&keys::job(&id), &spec).unwrap();
    eventually("job running", || a.state.running.lock().unwrap().contains_key(&id)).await;
    // Let it get past the settle window and actually start.
    eventually("claim settled", || a.state.store.get(&keys::claim(&id)).unwrap().is_some()).await;
    pause(a.state.cfg.settle_window() + Duration::from_secs(1)).await;
    spec.cancelled = true;
    a.state.store.put_json(&keys::job(&id), &spec).unwrap();
    eventually("result written", || a.state.store.get(&keys::result(&id)).unwrap().is_some()).await;
    let result: JobResult = a.state.store.get(&keys::result(&id)).unwrap().unwrap().parse().unwrap();
...
async fn desired_state_converges_and_reports() {
    ...
    let desired = DesiredState {
        files: vec![DesiredFile { path: target.to_string_lossy().into(), content: "hello\n".into(), mode: Some("0600".into()) }],
        ensure: vec![Ensure {
            name: "marker".into(),
    ...
    a.state.store.put_json(&keys::desired("id-solo"), &desired).unwrap();
    eventually("reconciled", || a.state.store.get(&keys::reconcile("id-solo")).unwrap().is_some()).await;
    eventually("file written", || std::fs::read_to_string(&target).ok().as_deref() == Some("hello\n")).await;
    ...
    eventually("converged", || {
        a.state.store.get(&keys::reconcile("id-solo")).unwrap().and_then(|r| r.parse::<ReconcileReport>().ok()).map(|r| r.converged).unwrap_or(false)
    })
    .await;
    std::fs::remove_dir_all(dir).ok();
}
